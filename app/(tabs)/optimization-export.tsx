    ...
    try {
      const data = {
        ...
        summary: {
          ...
          totalSavings: recommendations.filter(r => r.applied).reduce((sum, r) => sum + r.estimatedCostReduction, 0),
        },
      };

      let content = '';
      let filename = `optimization_report_${Date.now()}`;

      if (format === 'csv') {
        content = 'Type,Description,Priority,Estimated Savings (%),Cost Reduction ($),Applied\n';
        recommendations.forEach(r => {
          content += `${r.type},${r.description},${r.priority},${r.estimatedSavings},${r.estimatedCostReduction},${r.applied ? 'Yes' : 'No'}\n`;
        });
        filename += '.csv';
      } else if (format === 'json') {
        ...
        const totalSavings = recommendations.filter(r => r.applied).reduce((sum, r) => sum + r.estimatedCostReduction, 0);
...
<head>
  ...
  <style>
    ...
    .applied { background-color: #e8f5e9; }
  </style>
</head>
<body>
  ...
  <div class="summary">
    <h2>Summary</h2>
    <p>Applied Recommendations: ${appliedCount} of ${recommendations.length}</p>
    <p>Estimated Monthly Savings: $${totalSavings}</p>
  </div>
  <h2>Recommendations</h2>
  <table>
    <tr><th>Type</th><th>Description</th><th>Priority</th><th>Savings</th><th>Cost Reduction</th><th>Status</th></tr>
    ${recommendations.map(r => `<tr class="${r.applied ? 'applied' : ''}"><td>${r.type}</td><td>${r.description}</td><td>${r.priority}</td><td>${r.estimatedSavings}%</td><td>$${r.estimatedCostReduction}</td><td>${r.applied ? 'Applied' : 'Pending'}</td></tr>`).join('')}
  </table>
</body>
</html>`;
        filename += '.html';
    ...
    <ScreenContainer className="p-4">
      <ScrollView contentContainerStyle={{ flexGrow: 1 }}>
        <View className="gap-6">
          ...
          {/* Export Section */}
          <View className="gap-3">
            <Text className="text-lg font-semibold text-foreground">📥 Export Recommendations</Text>
